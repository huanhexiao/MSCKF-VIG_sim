...
public:
    ...
    bool ProcessImage(const std::string &ImgTxt, const double &time, utiltool::NavInfo &navinfo);
    void ReviseCameraState(const Eigen::VectorXd &dx_camera);

    bool GetMSCKFState(){return bMSCKF_DoneNow_;}
    bool CloseMSCKFState(){ bMSCKF_DoneNow_ = false;}

   private:
    void TrackSim(const std::string &ImgTxt);
    void ExtractMatches();
    void AguementedState(const utiltool::NavInfo &navinfo);
    void AddObservation();
    void DetermineMeasureFeature();
    bool CheckEnableTriangulate(const Feature &feature);
    bool CheckMotionStatus(const Feature &feature);
    bool LMOptimizePoint(Feature &feature);
    ...
    void Test();
    bool LMOptimizePointAndCheck(Feature &feature);
    bool jTriangulate(Feature &feature);
...
private:
    ...
    // 最新帧帧上提取、跟踪到的关键点序列，有新点会自动累加
    std::vector<FeatureId> keypoints_id_;
    // 这个点集是最新帧上，地图点序列与帧点序列关联
    // 两者数量应该相同，不同的是keypoints_关键点序号经过筛选后可能不连续
    std::map<FeatureId, FeatureId> keypoints_featureid_;
...
private:
    ...
    std::map<unsigned long long int, cv::Point2f> mppre_frame_keypoints_;
    std::map<unsigned long long int, cv::Point2f> mpcurr_frame_keypoints_;
    int Min_CoVis_Frames_ = 3, MaxHsize_ = 800;
    double FocalLength_ = 500.0;
