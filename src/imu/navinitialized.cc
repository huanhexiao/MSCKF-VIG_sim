...
// 旋转quat_,rotation_转到E下，别的不动。特别是姿态角att_仍然是N系下的
NavInfo &AttCaln2e(NavInfo &nav_info)
{
    ...
    nav_info.vel_ = earth::CalCn2e(blh(0), blh(1)) * nav_info.vel_;
                                   ...
                                   std::map<double, Eigen::Matrix<double, 9, 1>> &mpNavTrue)
{
    ...
    if (gnss_data->format_type_ == GNSSPOS)   // 有三种，另外两种是位置+速度、速度
    {
        // 继续遍历data序列，获取下一个GNSS，以便内插计算速度
        data = ptr_data_queue_->GetData();
        while (data->get_type() != GNSSDATA)
        {
            data = ptr_data_queue_->GetData();
    ...
    /*** 如果初始姿态是给定的，这里直接读出来就结束了 ***/
    if (use_set_attitude)
    {
        aligned_mode_ = SETTING;

        nav_info.time_ = gnss_data->get_time();
        assert(mpNavTrue.find(nav_info.time_) != mpNavTrue.end());
        Eigen::Matrix<double, 9, 1> InitState = mpNavTrue.at(nav_info.time_);

        nav_info.pos_ << InitState[0], InitState[1], InitState[2];
        nav_info.vel_ << InitState[3], InitState[4], InitState[5];
        nav_info.att_(0) = InitState[6] * constant::deg2rad;
        nav_info.att_(1) = InitState[7] * constant::deg2rad;
        nav_info.att_(2) = InitState[8] * constant::deg2rad;
        nav_info.rotation_ = Euler2RotationMatrix(nav_info.att_);
        nav_info.quat_ = RotationMartix2Quaternion(nav_info.rotation_);
        ...
        nav_info.pos_std_ << pos_std[0], pos_std[1], pos_std[2];
        nav_info.vel_std_ << vel_std[0], vel_std[1], vel_std[2];
        nav_info.att_std_ << att_std[0] * deg2rad, att_std[1] * deg2rad, att_std[2] * deg2rad;
        // 旋转quat_,rotation_转到E下，别的不动。特别是姿态角att_仍然是N系下的
