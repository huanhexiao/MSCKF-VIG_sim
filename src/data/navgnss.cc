...
void GetStartAndEndTime(double &start_time, double &end_time)
{
	...
	start_time = config->get<double>("start_time");
	end_time = config->get<double>("end_time");
}
...
void FileGnssData::ReadingData()
{
	...
	while (!ifs_file_.eof())
	{
		...
		try
		{
			double time(std::stod(dat[0]));
			if (time < start_time || time > end_time)
				continue;
			GnssData::Ptr gd_temp = std::make_shared<GnssData>();
			gd_temp->set_time(time);
			gd_temp->pos_ = Vector3d{std::stod(dat[1]), std::stod(dat[2]), std::stod(dat[3])};
			gd_temp->pos_std_ = Vector3d{std::stod(dat[4]) , std::stod(dat[5]) , std::stod(dat[6]) };
			if (std::stoi(dat[7]) == 1 && (!IsBreak(time)))
			{
				// if (logout_)
				// {
