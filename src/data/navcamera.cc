...
{
    utiltool::ConfigInfo::Ptr config = utiltool::ConfigInfo::GetInstance();
    std::string image_path = config->get<std::string>("image_path");
    ...
    while (!ifs_camera_data_config_.eof())
    {
        ...
        if(data.size()<2)
            continue;
        double time(std::stod(data[1]));
        if (time < start_time || time > end_time)
            continue;
        std::string ImgTxt = image_path + data[0] + ".txt";
        camera::CameraData::Ptr image = std::make_shared<camera::CameraData>(time, ImgTxt);
        {
            std::unique_lock<std::mutex> lck(mtx_collectdata_);
